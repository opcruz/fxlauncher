...
public class CreateManifest {
  private static final ArrayList<String> includeExtensions = new ArrayList<>();
  ...
  public static void main(String[] args) throws IOException, URISyntaxException {
    ...
    String progressLabelStyle = null;
    ...
    if (args.length > 3) {
      ...
      if (named != null) {
        ...
        // Configure progressLabelStyle
        if (named.containsKey("progress-label-style"))
          progressLabelStyle = named.get("progress-label-style");
      ...
      for (String raw : params.getRaw()) {
        ...
        if (raw.startsWith("--progress-label-style=")) continue;
    ...
    if (progressLabelStyle != null) manifest.progressLabelStyle = progressLabelStyle;
    ...
    if (stopOnUpdateErrors != null) {
      ...
      if (stopOnUpdateErrorsDeprecated != null) {
        System.out.println(
            "Warning: --stopOnUpdateErrors is deprecated. "
    ...
    // use --stopOnUpdateError if it was specified.
      ...
      System.out.println(
          "Warning: --stopOnUpdateErrors is deprecated. " + "Use --stop-on-update-errors instead.");
    }
    JAXB.marshal(manifest, appPath.resolve("app.xml").toFile());
  }

  public static FXManifest create(URI baseURI, String launchClass, Path appPath)
    ...
    if (!manifest.uri.getPath().endsWith("/")) {
      manifest.uri = new URI(String.format("%s/", baseURI));
    }
    Files.walkFileTree(
        appPath,
            new SimpleFileVisitor<>() {
              public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
                ...
                if (!Files.isDirectory(file)
                        && shouldIncludeInManifest(file)
                        && !file.getFileName().toString().startsWith("fxlauncher"))
   ...
   *
   * <p>Although the method is called setIncludeExtensions, it actually does an addAll.
   *
